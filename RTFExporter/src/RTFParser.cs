...
namespace RTFExporter
{
	...
	public class RTFParser
	{
		...
		/// <param name="documentToSave">The RTF document to save</param>
		public static void ToFile(
			string path,
			RTFDocument documentToSave)
		{
			documentToSave.SetFile(path);
			documentToSave.Save();
			documentToSave.Close();
		}
		...
		public static string ToString(
			RTFDocument sourceDocument)
		{
			document = sourceDocument;

			string str = "{\\rtf1\\ansi\\deff0";

			foreach (RTFParagraph paragraph in sourceDocument.paragraphs)
			{
			...
			str += "\\paperw" + Value(sourceDocument.width) + "\\paperh" + Value(sourceDocument.height) +
				   "\\margl" + Value(sourceDocument.margin.left) + "\\margr" + Value(sourceDocument.margin.right) +
				   "\\margt" + Value(sourceDocument.margin.top) + "\\margb" + Value(sourceDocument.margin.bottom) + " ";
		...
		private static readonly Dictionary<string, int> m_FontsIndex = new Dictionary<string, int>();
		...
		private static string ColorParsing()
		{
			...
			for (int i = 0; i < document.colors.Count; i++)
			{
				...
				{
					if (color.r == document.colors[i].r && color.g == document.colors[i].g &&
		...
		private static string ParseText(
			RTFText text)
		{
			string str = ParseTextFormatting(text);

			CleanTextContent(text);

			if (text is RTFHyperlink)
			{
				// Hyperlink format - formatting is in commands before the field.
		...
		private static void CleanTextContent(
			RTFText text)
		{
		...
		private static string ParseTextFormatting(
			RTFText text)
		{
			var str = "\\plain ";
			...
			str += "\\f" + m_FontsIndex[text.style.fontFamily] + " ";
